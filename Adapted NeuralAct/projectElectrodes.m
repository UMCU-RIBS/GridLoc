function [ subjstructs, M ] = projectElectrodes( M, subjstructs, normway, normUse, interstype, intersval)
...
%
%   CALLING SEQUENCE:
%   [ subjstructs ] = projectElectrodes( M, subjstructs, normway, normUse, interstype, intersval)
%
...
%      
%           normUse:        normUse set to 1, uses the normal which is
%           include in the subjstructs. If normUse is set to 0, it will
...
%   Author: Jan Kubanek, edited for speed by Michael Leibbrand
...
if normUse == 0
       ...
       %compute the normal vector
           ...
           p2 = p1 + normal;
           subjstructs(subj).normal(eg,:) = normal;
...
else %in case you want to use a different normal (previously calculated)
    p1 = subjstructs(subj).electrodes(eg,:);
    p2 = p1+subjstructs(subj).normal(eg,:);
end
