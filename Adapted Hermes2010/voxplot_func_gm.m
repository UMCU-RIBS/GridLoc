function [xyztCortex,t_surf] = voxplot_func_gm(sName, tName, cname, Tthreshold,Dthreshold,varargin)
%
% use as follows:
...
xyzs=(xyzs*s_info.mat(1:3,1:3)')+repmat(s_info.mat(1:3,4),1,length(xyzs))'; %transformation from voxels into mm

% indices to native tmap
[xt,yt,zt]=ind2sub(size(t),find(t>=Tthreshold));
xyzt=[xt yt zt];
clear xt yt zt % housekeeping
xyzt=(xyzt*t_info.mat(1:3,1:3)')+repmat(t_info.mat(1:3,4),1,length(xyzt))'; %transformation from voxels into mm
